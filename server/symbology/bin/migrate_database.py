...
Usage:
    python -m symbology.bin.copy_database --help
    python -m symbology.bin.copy_database --source dev --target staging
    python -m symbology.bin.copy_database --dry-run
"""
...
Examples:
  ...
  python -m symbology.bin.copy_database --source dev --target staging-1g

  # Migrate to specific IP address
  python -m symbology.bin.copy_database --source dev --target 10.0.0.21

  # Migrate with auto-confirmation (for automation)
  python -m symbology.bin.copy_database --source dev --target staging -y

  # Dry run to see what would happen
  python -m symbology.bin.copy_database --source dev --target staging --dry-run

  # Check status of both databases
  python -m symbology.bin.copy_database --status-only
        """
    )
