...
from symbology.database.documents import Document, get_document_by_content_hash
import symbology.database.generated_content as db
from symbology.database.model_configs import get_model_config_by_content_hash
from symbology.database.prompts import create_prompt, get_prompt_by_content_hash, PromptRole
...
def create_generated_content(prompt: str, model_config: str, source_documents: Tuple[str],
    ...
    if output == 'json':
        ...
        original_levels = {}
        logger_names = [
            'symbology.database.generated_content',
            'symbology.database.prompts',
            'symbology.database.model_configs',
            'symbology.database.documents',
            'symbology.database.companies',
...
def get_generated_content(hash: str, show_full: bool, output: str):
    ...
    if output == 'json':
        # Get all loggers and set their level to WARNING temporarily
        original_levels = {}
        for logger_name in ['symbology.database.generated_content']:
            db_logger = logging.getLogger(logger_name)
            original_levels[logger_name] = db_logger.level
    ...
    try:
        ...
        if output == 'json':
            ...
            content_data = {
                ...
                "input_tokens": content_obj.input_tokens,
                "output_tokens": content_obj.output_tokens,
