...
def test_ingest_company_happy_path():
    ...
    with mock.patch('symbology.ingestion.ingestion_helpers.get_company', return_value=mock_edgar_company) as mock_get_company, \
         mock.patch('symbology.ingestion.ingestion_helpers.upsert_company_by_cik', return_value=mock_db_company) as mock_upsert_company, \
...
def test_ingest_company_invalid_fiscal_year_format():
    ...
    with mock.patch('symbology.ingestion.ingestion_helpers.get_company', return_value=mock_edgar_company), \
         mock.patch('symbology.ingestion.ingestion_helpers.upsert_company_by_cik', return_value=mock_db_company) as mock_upsert_company, \
         mock.patch('symbology.ingestion.ingestion_helpers.logger') as mock_logger:
