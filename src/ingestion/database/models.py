...
# Association table for LLMCompletion to SourceDocument many-to-many relationship
...
# Association table for LLMCompletion to LLMCompletion many-to-many self-referential relationship
...
class SourceDocument(Base):
    ...
    # Relationship with LLMCompletions that use this document as context
...
class PromptTemplate(Base):
    ...
    completions = relationship("LLMCompletion", back_populates="prompt_template", cascade="all, delete-orphan")
...
class LLMCompletion(Base):
    ...
    def __repr__(self):
        return f"<LLMCompletion(id={self.id}, prompt_template_id={self.prompt_template_id}, created_at={self.created_at})>"

    def to_dict(self):
        """Convert LLMCompletion object to dictionary"""
        result = {c.name: getattr(self, c.name) for c in self.__table__.columns}
        return result

    def to_json(self):
        """Convert LLMCompletion object to JSON string"""
        import json
        return json.dumps(self.to_dict(), default=str)
...
class CompletionRating(Base):
    ...
    completion = relationship("LLMCompletion", back_populates="ratings")
