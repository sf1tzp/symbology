...
def process_cover_page_dataframe(
    company_id: int,
    filing_id: int,
    df: pd.DataFrame,
    session=None
) -> Dict[str, Any]:
    """Process a cover page dataframe and store values in the database.

    Args:
        company_id: ID of the company
        filing_id: ID of the filing
        df: Dataframe from get_cover_page_values()
        session: Database session (optional)
        ...
        for date_col in date_columns:
            ...
            if date_str in row and not pd.isna(row[date_str]) and row[date_str] != '':
                try:
                    ...
                    logger.warning(f"Could not convert value '{row[date_str]}' to float for concept {concept_id}")
                    continue
...
def store_cover_page_data(edgar_filing: Any, db_company: Company, db_filing: Filing, session: Optional[Session] = None) -> Dict[str, Any]:
    """
    Extract cover page data from an EDGAR filing and store it in the database.
    ...
    try:
        ...
        from .ingestion.edgar import get_cover_page_values
