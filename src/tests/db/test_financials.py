...
class TestFinancialData:
    ...
    @pytest.fixture
    def sample_cover_page_data(self):
    ...
    def test_process_cover_page_dataframe(self, db_session, test_company, test_filing, sample_cover_page_df):
        ...
        # Verify results
        assert results["total_concepts"] == 1  # Three concepts in the sample dataframe
        ...
        # Check that concepts have correct labels
        entity_name_concept = db_session.query(FinancialConcept).filter(
            FinancialConcept.concept_id == "dei_EntityCommonStockSharesOutstanding"
        ).first()
        assert "Entity Common Stock, Shares Outstanding" in entity_name_concept.labels
