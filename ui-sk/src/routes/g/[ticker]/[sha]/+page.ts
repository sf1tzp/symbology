...
import { getCompanyByTicker, getGeneratedContentByTickerAndHash, getModelConfigById, getDocumentById, getGeneratedContentById } from '$lib/api';
        ...
        if (content && company) {
            ...
            if (content.source_document_ids && content.source_document_ids.length > 0) {
                try {
                    ...
                    const documentSources = documents.filter(doc => doc !== null).map(doc => ({
                        ...doc,
            ...
            if (content.source_content_ids && content.source_content_ids.length > 0) {
                try {
                    ...
                    const contentSources = contents.filter(content => content !== null).map(content => ({
                        ...content,
