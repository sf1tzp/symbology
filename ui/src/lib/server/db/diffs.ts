...
export async function loadFilingRefs(
	ids: (string | null)[],
...
export async function loadSummaries(ids: (string | null)[]): Promise<Map<string, string | null>> {
	const real = ids.filter((x): x is string => x !== null);
	if (real.length === 0) return new Map();
	const rows = await db
...
// Landing showcase bounds: a featured diff must read as a genuine *prose*
...
// generated summary reuses the pipeline's own significance gate. Consumed by
// getLandingShowcase (landing.ts).
...
export function hasProseEdit(ops: DiffOp[]): boolean {
	const changed = ops
		.filter((o) => o.op !== 'equal')
		.map((o) => o.text)
		.join('');
...
export function isCleanHeading(heading: string | null): heading is string {
