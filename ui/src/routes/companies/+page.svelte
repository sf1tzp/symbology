<script lang="ts">
	...
	let featured = $state<CompanyListItem[]>([]);
	...
	const showingFrom = $derived(currentPage * PAGE_SIZE + 1);
	const showingTo = $derived(Math.min((currentPage + 1) * PAGE_SIZE, total));
	...
	<div class="flex-between" style="margin-bottom: 18px;">
		...
		{#if total > 0}
			<span class="meta" style="color: var(--ink-4);">
				{total.toLocaleString()} results &middot; showing {showingFrom}&ndash;{showingTo}
			</span>
		{/if}
	</div>
